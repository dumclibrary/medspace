...
describe Medspace::Importer do
  ...
  let(:input_file) { file_fixture('importer/external_object/sample_medspace_data.xml') }
  let(:data_path) { Rails.root.join('spec', 'fixtures', 'files', 'importer', 'external_object') }
  let(:msi_invalid) { described_class.new(input_file_with_no_title, data_path) }
  let(:input_file_with_no_title) { file_fixture('importer/external_object/invalid_medspace_data.xml') }
  ...
  context 'processing a single record' do
    ...
    it 'does not create a Fedora object if one already exists with same identifier' do
      ...
      number_of_works = ExternalObject.where(identifier: ExternalObject.last.identifier).count

      expect(number_of_works).to eq 1
    end

    it 'creates a Fedora record from a Medspace object' do
      expect(work).to be_instance_of(ExternalObject)
    end
  ...
  context 'processing multiple records' do
    ...
    describe 'import' do
      ...
      it 'logs an error message with a filename from invalid xml' do
        expect { msi_invalid.import }.to output(/XML\: spec\/fixtures\/files\/importer\/external_object\/invalid_medspace_data.xml is invalid/).to_stdout_from_any_process
      end

      context 'when some records fail to import' do
        let(:input_file_1) { file_fixture('importer/external_object/no_based_near_data.xml') }
        let(:input_file_2) { file_fixture('importer/external_object/no_subject_data.xml') }
        let(:input_file_3) { file_fixture('importer/external_object/sample_medspace_data.xml') }
        let(:input_file_4) { file_fixture('importer/external_object/complete_medspace_data.xml') }
        let(:model) { ExternalObject }
